import { CognitoDao } from "../aws-cognito/CognitoDao";
...
export class UserAccountManager {
    ...
    async signUp(rawPassword: string, email: string) {
        ...
        if (!cognitoUserUUid) {
            ...
            throw new Error(
                "Cognito returned undefined for the user sub value."
            );
        }
    ...
    async confirmForgottenPassword(
        ...
        if (user !== null) {
            ...
            try {
                ...
                console.warn(
                    "Rolling backing user password update from database"
                );
                ...
                try {
                    ...
                    console.error(
                        `Failed to resave old password for user uuid: ${uuid}`
                    );
                }
            }
            ...
            const newPassword =
                this.userFactory.createUserPassword(newRawPassword);
            try {
                await this.userRepo.update(uuid, {
    ...
    async deleteUser(uuid: string) {
        ...
        if (user !== null) {
            try {
                ...
                console.error(
                    `Failed to delete user from aws cognito uuid: ${uuid}`
                );
                throw error;
            }
            ...
            try {
                ...
                console.error(
                    `Failed to delete user from database uuid: ${uuid}`
                );
                throw error;
            }
        }
